...
const App = () => {
    ...
    useEffect(() => {
        ...
        styleTag.innerHTML = `
            ...
            .mobile-nav-btn { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; flex: 1; height: 100%; transition: all 0.2s; color: #52525b; border: none; background: transparent; }
            .mobile-nav-btn.active { color: var(--brand-primary); }
            .mobile-nav-label { font-size: 7px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.05em; }
        `;
    }, [theme]);
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-[#f8f9fa] relative">
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                ...
                <div className="flex-1 overflow-auto p-4 lg:p-8 space-y-6 pb-20 lg:pb-8">
                    {view === 'DASHBOARD' && (
                ...
                {/* BOTTOM NAV - KOMPAKT H-14 */}
                <div className="lg:hidden fixed bottom-0 left-0 right-0 h-14 bg-zinc-950 border-t border-zinc-900 flex items-center justify-around z-[210] px-1 pb-safe backdrop-blur-xl">
    ...
    return (
        <div className="fixed inset-0 bg-black flex items-center justify-center font-mono z-[300]">
            <form onSubmit={handleLogin} className="w-full max-w-sm p-8 bg-zinc-950 border border-zinc-900 space-y-6 text-white shadow-2xl">
                ...
                <button type="submit" className="w-full theme-bg text-black font-black py-5 text-[10px] uppercase tracking-[0.3em] hover:bg-white transition-colors">Connect</button>
            </form>
        </div>
    );
};
