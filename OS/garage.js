// garage.js - "Precision & Density" V24 (Restored Geometry & Fixed Colors)
...
// --- IKONER ---
// Här skiljer vi nu strikt på "Linje-ikoner" (Stroke/24px) och "Fyllda ikoner" (Fill/Specialstorlek)
// Detta garanterar att både kryss och motor syns korrekt.
const SafeIcon = ({ name, size = 16, className = "" }) => {
    const s = size;
    const c = className;

    switch (name) {
        // --- 1. LINJE-IKONER (Används för UI, X, Pilar) ---
        // Dessa ska ha stroke="currentColor" och fill="none"
        ...
        // --- 2. FYLLDA IKONER (Används för Fordonsdata) ---
        // Dessa har specifika ViewBoxes och ska ha fill="currentColor" och stroke="none"
...
// --- KOMPONENT: FORDONSAKT ---
    ...
    const [histQ, setHistQ] = React.useState(""); // Sökterm för historik
    ...
    const fields = [
        ...
        { id: 'oil', label: 'Motorolja', icon: 'oil', ph: '4.3l 0w30', action: 'oil_search' }, // Action flagga
            ...
            <div onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd} className="relative w-full sm:w-[500px] h-full bg-[#f8f8f8] sm:bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 border-l border-zinc-800">
                ...
                <div className="bg-[#111113] text-white shrink-0 relative overflow-hidden shadow-md z-20">
                    ...
                    <div className="flex justify-between items-start p-5 pb-2 relative z-10">
                        ...
                        {/* FIX: Text-white för stäng-krysset */}
                        <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-all z-50 shadow-sm">
                            <svg viewBox="0 0 24 24" width="14" height="14" stroke="white" strokeWidth="3" fill="none" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    {activeTab === 'HISTORY' ? (
                        <div className="space-y-4 animate-in fade-in duration-300 pb-20 relative">
                            ...
                            <div className="relative mb-2">
                                <input 
                                    type="text" 
                                    placeholder="Sök i historik (t.ex. broms, service)..." 
                                    value={histQ}
                                    onChange={(e) => setHistQ(e.target.value)}
                                    className="w-full bg-white border border-zinc-200 rounded p-2 pl-9 text-[11px] font-bold uppercase tracking-wide focus:outline-none focus:border-orange-500"
                                />
                                <span className="absolute left-3 top-2.5 text-zinc-400"><SafeIcon name="search" size={12}/></span>
                            </div>

                            {/* TIDSLINJEN & KORTEN (Endast en container med linje) */}
                                ...
                                {filteredHistory.map((j, index) => {
                                    ...
                                    return (
                                        <div key={j.id} className="relative pl-6 group">
                                            ...
                                            {/* DEN EXAKTA ORANGEA LINJEN (broar gapet ner till nästa) */}
                                            {isHighlighted && (
                                                <div 
                                            ...
                                            {/* PRICKEN PÅ TIDSLINJEN */}
                                            <div className={`absolute -left-[5px] top-4 w-2.5 h-2.5 rounded-full border-2 shadow-sm z-10 transition-colors ${isHighlighted ? 'bg-orange-500 border-orange-500' : (['KLAR','FAKTURERAS'].includes(j.status)?'bg-emerald-500 border-white':'bg-zinc-200 border-white')}`}></div>

                                            {/* SJÄLVA KORTET (Ett enda rent kort) */}
                                            <div 
                                                onClick={()=>setView('NEW_JOB',{job:j})} 
                                                className={`cursor-pointer rounded-md p-4 transition-all duration-300 relative z-10 ${
                                                    isHighlighted 
                                                    ? 'bg-white border-2 border-orange-500 shadow-[0_0_15px_rgba(249,115,22,0.15)]' 
                                                    : 'bg-white border border-zinc-200 hover:border-zinc-300 shadow-sm'
                                                }`}
                                            >
                                                {/* Datum & Pris */}
                                                <div className="flex justify-between items-baseline mb-2">
                                                    <div className={`font-mono text-[11px] font-black ${isHighlighted ? 'text-orange-600' : 'text-zinc-500'}`}>
                                                        {j.datum ? j.datum.split('T')[0] : 'Inväntar'}
                                                    </div>
                                                    <div className={`font-mono text-[13px] font-black ${isHighlighted ? 'text-zinc-900' : 'text-zinc-700'}`}>
                                                        {parseInt(j.kundpris||0).toLocaleString()} <span className="text-[10px] text-zinc-400 font-sans">kr</span>
                                                    </div>
                                                </div>
                                                
                                                {/* Kundnamn & Status */}
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className={`text-[12px] font-black uppercase tracking-tight ${isHighlighted ? 'text-zinc-900' : 'text-zinc-800'}`}>
                                                        {j.kundnamn}
                                                    </div>
                                                ...
                                                {/* Kommentar */}
                                                <p className={`text-[11px] leading-relaxed line-clamp-2 ${isHighlighted ? 'text-orange-800' : 'text-zinc-500'}`}>
                                        ...
                                        <div className="flex-1 overflow-hidden relative">
                                            ...
                                            {f.action === 'oil_search' && (
                                                <a 
                                                    ...
                                                    target="_blank" 
                                                    rel="noreferrer"
                                                    className="absolute right-0 top-3 p-1 bg-orange-100 text-orange-600 rounded hover:bg-orange-200 active:scale-95 transition-all" 
                                                    title="Sök Castrol/Mobil1 Olja"
                                                >
...
window.VehicleProfileLoader = ({ regnr, highlightId, onClose, setView }) => {
    ...
    // Skicka med highlightId in i VehicleProfile!
