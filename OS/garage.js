// garage.js - "Precision & Density" V23 (Icons Hard-Fixed)
...
// --- IKONER (Hårdkodade för att garantera visning) ---
const SafeIcon = ({ name, size = 16, className = "" }) => {
    // Standardinställningar för "linje-ikoner" (Feather style)
    const lineProps = { xmlns: "http://www.w3.org/2000/svg", width: size, height: size, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", className };
    
    // Inställningar för "fyllda ikoner" (Motor, etc)
    const fillProps = (vb) => ({ xmlns: "http://www.w3.org/2000/svg", width: size, height: size, viewBox: vb, fill: "currentColor", stroke: "none", className });

    switch (name) {
        // UI Standard
        case 'db': return <svg {...lineProps}><path d="M3 5c0-1.1 4.5-2 10-2s10 .9 10 2a2 2 0 0 1 0 .6l-8.3 4.7a3.5 3.5 0 0 1-3.4 0L3 5.6A2 2 0 0 1 3 5zm0 6c0-1.1 4.5-2 10-2s10 .9 10 2a2 2 0 0 1 0 .6l-8.3 4.7a3.5 3.5 0 0 1-3.4 0L3 11.6A2 2 0 0 1 3 11zm0 6c0-1.1 4.5-2 10-2s10 .9 10 2a2 2 0 0 1 0 .6l-8.3 4.7a3.5 3.5 0 0 1-3.4 0L3 17.6A2 2 0 0 1 3 17z" /></svg>;
        case 'x': return <svg {...lineProps}><path d="M18 6 6 18M6 6l12 12" /></svg>;
        case 'search': return <svg {...lineProps}><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>;
        case 'plus': return <svg {...lineProps}><path d="M5 12h14M12 5v14" /></svg>;
        case 'right': return <svg {...lineProps}><path d="m9 18 6-6-6-6" /></svg>;
        case 'clock': return <svg {...lineProps}><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>;
        case 'trend': return <svg {...lineProps}><polyline points="23 6 13.5 15.5 8.5 10.5 1 18" /><polyline points="17 6 23 6 23 12" /></svg>;
        case 'ext': return <svg {...lineProps}><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>;
        case 'edit': return <svg {...lineProps}><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" /><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" /></svg>;
        case 'hist': return <svg {...lineProps}><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>;

        // Teknisk Data (Dessa var de som försvann - nu hårdkodade korrekt)
        case 'car': return <svg {...lineProps}><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H7.7c-.7 0-1.3.3-1.8.7C5 8.6 3.7 10 3.7 10s-2.7.6-4.5 1.1C-.3 11.3 0 12.1 0 13v3c0 .6.4 1 1 1h2" /><circle cx="7" cy="17" r="2" /><circle cx="17" cy="17" r="2" /></svg>;
        
        case 'ac': return <svg {...lineProps}><path d="M12 3v18"/><path d="M3 12h18"/><path d="M5.6 5.6l12.8 12.8"/><path d="M18.4 5.6L5.6 18.4"/></svg>;

        case 'oil': return <svg {...fillProps("0 0 24 24")}><path fill="none" d="M0 0h24v24H0z"></path><path d="M8 5h11a1 1 0 0 1 1 1v15a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V11l4-6zm5-4h5a1 1 0 0 1 1 1v2h-7V2a1 1 0 0 1 1-1zM6 12v7h2v-7H6z"></path></svg>;

        case 'eng': return <svg {...fillProps("0 -14.14 122.88 122.88")}><path d="M43.58,92.2L31.9,80.53h-8.04c-2.81,0-5.11-2.3-5.11-5.11v-8.7h-4.87V76.9c0,2.17-1.78,3.95-3.95,3.95H3.95 C1.78,80.85,0,79.07,0,76.9V42.4c0-2.17,1.78-3.95,3.95-3.95h5.98c2.17,0,3.95,1.78,3.95,3.95v10.18h4.87v-9.36 c0-2.81,2.3-5.11,5.11-5.11h8.54l12.07-12.83c1.4-1.22,3.26-1.65,5.43-1.56h49.73c1.72,0.19,3.03,0.85,3.83,2.09 c0.8,1.22,0.67,1.91,0.67,3.28v23.49H109V42.4c0-2.17,1.78-3.95,3.95-3.95h5.98c2.17,0,3.95,1.78,3.95,3.95v34.5 c0,2.17-1.78,3.95-3.95,3.95h-5.98c-2.17,0-3.95-1.78-3.95-3.95V66.72h-4.87v0.92c0,2.73,0.08,4.38-1.66,6.64 c-0.33,0.43-0.7,0.84-1.11,1.22L83.53,92.96c-0.89,0.99-2.24,1.53-4.02,1.63h-30.4C46.84,94.49,44.99,93.71,43.58,92.2L43.58,92.2z M63.71,61.78l-12.64-1.19l10.48-22.96h14.33l-8.13,13.17l14.62,1.62L55.53,84.64L63.71,61.78L63.71,61.78z M51.98,0h34.5 c2.17,0,3.95,1.78,3.95,3.95v5.98c0,2.17-1.78,3.95-3.95,3.95H76.3v5.03H62.16v-5.03H51.98c-2.17,0-3.95-1.78-3.95-3.95V3.95 C48.03,1.78,49.81,0,51.98,0L51.98,0z"></path></svg>;

        case 'belt': return <svg {...fillProps("0 0 1024 1024")}><path d="M512 896a384 384 0 1 0 0-768 384 384 0 0 0 0 768zm0 64a448 448 0 1 1 0-896 448 448 0 0 1 0 896z"></path><path d="M192 512a320 320 0 1 1 640 0 32 32 0 1 1-64 0 256 256 0 1 0-512 0 32 32 0 0 1-64 0z"></path><path d="M570.432 627.84A96 96 0 1 1 509.568 608l60.992-187.776A32 32 0 1 1 631.424 440l-60.992 187.776zM502.08 734.464a32 32 0 1 0 19.84-60.928 32 32 0 0 0-19.84 60.928z"></path></svg>;

        case 'trq': return <svg {...fillProps("0 0 1024 1024")}><path d="M865.3 244.7c-.3-.3-61.1 59.8-182.1 180.6l-84.9-84.9 180.9-180.9c-95.2-57.3-217.5-42.6-296.8 36.7A244.42 244.42 0 0 0 419 432l1.8 6.7-283.5 283.4c-6.2 6.2-6.2 16.4 0 22.6l141.4 141.4c6.2 6.2 16.4 6.2 22.6 0l283.3-283.3 6.7 1.8c83.7 22.3 173.6-.9 236-63.3 79.4-79.3 94.1-201.6 38-296.6z"></path></svg>;

        case 'bat': return <svg {...fillProps("0 0 512 512")}><path d="M432,132V60h-96v72H176V60H80v72H0v320h512V132H432z M181.156,258.063H82.719V227.5h98.438V258.063z M436,258.063h-33.938V292H371.5v-33.938h-33.938V227.5H371.5v-33.938h30.563V227.5H436V258.063z"></path></svg>;

        case 'wgt': return <svg {...fillProps("0 0 512 512")}><path d="M256 46c-45.074 0-82 36.926-82 82 0 25.812 12.123 48.936 30.938 64H128L32 480h448l-96-288h-76.938C325.877 176.936 338 153.812 338 128c0-45.074-36.926-82-82-82zm0 36c25.618 0 46 20.382 46 46s-20.382 46-46 46-46-20.382-46-46 20.382-46 46-46zm-82.215 202.95h23.5v33.263l33.873-33.264h27.283l-43.883 43.15 48.4 47.974H233.54l-36.255-35.888v35.888h-23.5V284.95zm119.934 21.24c4.76 0 8.952.934 12.573 2.806 3.62 1.872 6.938 4.82 9.95 8.85v-10.13h21.972v61.462c0 10.986-3.48 19.368-10.438 25.146-6.917 5.82-16.968 8.727-30.152 8.727-4.272 0-8.4-.325-12.39-.976-3.986-.65-7.996-1.647-12.024-2.99v-17.03c3.826 2.198 7.57 3.826 11.23 4.884 3.664 1.098 7.347 1.648 11.05 1.648 7.162 0 12.41-1.566 15.746-4.7 3.337-3.132 5.006-8.035 5.006-14.708v-4.7c-3.01 3.986-6.328 6.916-9.95 8.788-3.62 1.87-7.813 2.808-12.573 2.808-8.343 0-15.238-3.275-20.69-9.826-5.453-6.592-8.18-14.974-8.18-25.146 0-10.214 2.727-18.576 8.18-25.086 5.452-6.55 12.347-9.827 20.69-9.827zm8.118 15.746c-4.517 0-8.038 1.67-10.56 5.005-2.523 3.338-3.784 8.058-3.784 14.162 0 6.266 1.22 11.026 3.662 14.28 2.442 3.215 6.003 4.823 10.682 4.823 4.557 0 8.096-1.67 10.62-5.006 2.522-3.337 3.784-8.036 3.784-14.098 0-6.104-1.262-10.824-3.785-14.16-2.523-3.337-6.062-5.006-10.62-5.006z"></path></svg>;

        default: return <svg {...lineProps}><circle cx="12" cy="12" r="2" /></svg>;
    }
};
...
const VehicleProfile = ({ v, onClose, setView }) => {
    ...
    // SWIPE: Dra Vänster (<-) går till HÖGER flik. Dra Höger (->) går till VÄNSTER flik.
    ...
    const handleTouchEnd = (e) => {
        ...
        if (diff < -60 && activeTab === 'TECH') setActiveTab('HISTORY'); // Dra vänster -> Historik
        else if (diff > 60) {
            if (activeTab === 'HISTORY') setActiveTab('TECH'); // Dra höger -> Tech
            else onClose(); // Dra höger på Tech -> Stäng
        }
        tStart.current = null;
    };
    ...
    const fields = [
        ...
        { id: 'oil', label: 'Motorolja', icon: 'oil', ph: '4.3l 0w30' },
                    ...
                    {activeTab === 'HISTORY' ? (
                        ...
                        <div className="space-y-4 animate-in fade-in duration-300 pb-20">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {fields.map(f => (
                                    <div key={f.id} className="bg-white border border-zinc-200 p-2.5 flex items-center gap-3 shadow-sm rounded-[2px] hover:border-zinc-300 relative group-focus-within:border-orange-400">
                                        <div className="w-8 h-8 flex items-center justify-center text-zinc-400 shrink-0"><SafeIcon name={f.icon} size={18}/></div>
                                        <div className="flex-1 overflow-hidden">
                                            <div className="text-[9px] font-bold text-zinc-400 uppercase tracking-wide leading-none mb-1">{f.label}</div>
                                            {f.readOnly ? (
                                                <div className="text-[12px] font-bold text-zinc-900 truncate h-5">{f.val}</div>
                                            ) : (
                                                <input 
                                                    type="text" 
                                                    className="w-full text-[12px] font-bold text-zinc-900 bg-transparent outline-none placeholder:text-zinc-300 placeholder:font-normal"
                                                    placeholder={f.ph}
                                                    value={specs[f.id] || ''}
