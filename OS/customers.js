...
// Snygg Svensk Regskylt Komponent (Med hantering för okända bilar)
...
window.CustomersView = ({ allJobs, setView, setEditingJob, viewParams }) => {
    ...
    const RankBadge = ({ rank }) => {
        const colors = {
            'Premium': 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-[0_0_10px_rgba(249,115,22,0.4)] border border-orange-400',
            'Guld': 'bg-zinc-900 dark:bg-white/10 text-orange-400 border border-orange-500/30',
            'Silver': 'bg-zinc-100 dark:bg-white/5 text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-white/10',
            'Standard': 'bg-transparent text-zinc-400 border border-zinc-200 dark:border-white/5 opacity-80'
        };
        return (
            <span className={`text-[9px] font-black px-2 py-0.5 rounded shadow-sm tracking-widest uppercase flex items-center gap-1 ${colors[rank] || colors['Standard']}`}>
    ...
    if (selectedCustomer) {
        ...
        return (
            // FIX: Vänster-justerat (ml-0), tog bort mx-auto.
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 pb-6 border-b border-zinc-200 dark:border-white/5 gap-4 px-4 pt-5 lg:px-0 lg:pt-0">
                    <div className="flex items-center gap-4 md:gap-5">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-[10px] md:text-[11px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest mt-1.5 flex items-center gap-2">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
                                {displayedVehicles.map(v => (
                                    <div key={v} className="bg-zinc-50/80 dark:bg-[#1a2235] border border-zinc-200 dark:border-white/5 p-3 rounded-xl hover:border-orange-500/50 hover:shadow-sm transition-all group flex flex-col justify-center gap-2 min-w-0">
                                        ...
                                        {vehicleModels[v] && (
                                            /* FIX: Truncate och min-w-0 för att säkra layouten mot enorma modellnamn */
                        ...
                        <div className="bg-white/80 dark:bg-[#182032]/80 backdrop-blur-xl border border-zinc-200/80 dark:border-white/5 rounded-3xl shadow-sm overflow-hidden flex flex-col">
                            <div className="bg-zinc-50/50 dark:bg-[#1a2235]/50 p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-200/50 dark:border-white/10">
                                ...
                                <div className="relative group">
                                    <input 
                                        ...
                                        className="bg-white dark:bg-[#1a2235] border border-zinc-200 dark:border-white/10 text-[11px] font-bold text-zinc-900 dark:text-white px-10 py-3 outline-none focus:border-orange-500 w-full md:w-64 uppercase tracking-widest rounded-xl transition-all shadow-sm"
                                        value={logSearch}
                            ...
                            <div className="max-h-[700px] overflow-y-auto custom-scrollbar p-4 space-y-3">
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-start justify-between mb-3 border-b border-zinc-100 dark:border-white/5 pb-3 min-w-0">
                                                {/* FIX: min-w-0 lagt på flex containers för att möjliggöra snygg avklippning av långa bilnamn */}
                                ...
                                {filteredLogs.length > historyVisible && (
                                    ...
                                    >
                                        <SafeIcon name="refresh-cw" size={12} />
                                        Ladda fler uppdrag ({filteredLogs.length - historyVisible} kvar)
                                    </button>
                                )}
                            </div>
                        </div>
    ...
    return (
        // FIX: max-w-[1400px] och ml-0 för att hålla listan till vänster närmast sidomenyn.
        <div className="relative max-w-[1400px] w-full animate-in fade-in slide-in-from-left-4 duration-700 pb-0 ml-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 pb-6 border-b border-zinc-200 dark:border-white/5 gap-4 px-4 pt-5 lg:px-0 lg:pt-0">
                <div className="flex items-center gap-4 md:gap-5">
                    ...
                    <div className="flex flex-col">
                        <h1 className="text-2xl md:text-3xl font-black text-black dark:text-white uppercase tracking-tight leading-none drop-shadow-sm dark:drop-shadow-none">
                            KUND<span className="text-zinc-500 dark:text-zinc-500 font-light">REGISTER</span>
                        </h1>
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 z-10">
                    <div className="flex bg-white dark:bg-[#1a2235] p-1 border border-zinc-200 dark:border-white/5 rounded-xl shadow-sm">
                        {[
                            { id: 'revenue', icon: 'dollar-sign', tooltip: 'Omsättning' },
                            { id: 'count', icon: 'hash', tooltip: 'Antal Besök' },
                            { id: 'recent', icon: 'clock', tooltip: 'Senaste Besök' }
                        ].map(m => (
                            <button 
                                key={m.id}
            ...
            {/* KUNDLISTAN */}
            <div className="bg-white/80 dark:bg-[#182032]/80 backdrop-blur-xl rounded-2xl shadow-sm border border-zinc-200/80 dark:border-white/5 overflow-hidden flex flex-col mx-4 lg:mx-0 mb-8 relative">
                
                {/* FIX: Helt omstrukturerade width-klasser för att undvika att Klassificering krockar med Fordon */}
                <div className="hidden md:flex items-center px-6 py-4 bg-zinc-50/50 dark:bg-black/20 border-b border-zinc-200 dark:border-white/10 text-[10px] uppercase tracking-widest font-bold text-zinc-400 dark:text-zinc-500">
                    <div className="w-[30%] pl-1">Kund</div>
                    <div className="w-[15%]">Klassificering</div>
                    <div className="w-[20%]">Uppdrag & Fordon</div>
                    <div className="w-[15%] text-right">Omsättning</div>
                    <div className="w-[20%] text-right pr-12">Senast Aktiv</div>
                </div>

                <div className="flex flex-col relative">
                    {customerData.length === 0 ? (
                        <div className="p-16 text-center text-zinc-400 uppercase tracking-widest text-[11px] font-bold">
                            <SafeIcon name="users" size={40} className="mb-4 opacity-20 mx-auto" />
                        ...
                        <>
                            {visibleCustomers.map((customer, i) => {
                                ...
                                const topVehicle = customer.topVehicle;

                                return (
                                    <div 
                                        key={i} 
                                        onClick={() => { setSelectedCustomer(customer); setShowAllVehicles(false); }}
                                        className="group flex flex-col md:flex-row md:items-center justify-between p-4 md:px-6 md:py-4 bg-transparent hover:bg-zinc-50 dark:hover:bg-[#1f2940] cursor-pointer transition-colors border-b border-zinc-100 dark:border-white/5 last:border-0 relative overflow-hidden"
                                    >
                                        <div className="md:hidden flex flex-col gap-3 w-full">
                                            <div className="flex items-center justify-between w-full">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0 border ${avatarTheme}`}>
                                                        {initials}
                                                    </div>
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="text-[14px] font-bold text-zinc-900 dark:text-white truncate flex items-center gap-1.5">
                                                            <span className="truncate">{customer.name}</span>
                                                            {customer.rank === 'Premium' && <SafeIcon name="crown" size={12} className="text-orange-500 shrink-0" />}
                                                        </span>
                                            ...
                                            <div className="grid grid-cols-3 gap-2 bg-zinc-50 dark:bg-[#1a2235] p-3 rounded-xl border border-zinc-200 dark:border-white/5">
                                                <div className="flex flex-col items-center justify-center border-r border-zinc-200 dark:border-white/10">
                                                    <span className="text-[14px] font-light text-zinc-900 dark:text-white">{customer.missionCount}</span>
                                                    <span className="text-[9px] font-bold text-zinc-400 uppercase">Besök</span>
                                                </div>
                                                <div className="flex flex-col items-center justify-center border-r border-zinc-200 dark:border-white/10">
                                                    <span className="text-[14px] font-light text-zinc-900 dark:text-white">{(customer.totalSpent / 1000).toFixed(1)}k</span>
                                                    <span className="text-[9px] font-bold text-zinc-400 uppercase">Omsätt.</span>
                                                </div>
                                                <div className="flex flex-col items-center justify-center min-w-0 px-1">
                                                    <LicensePlate regnr={topVehicle} size="sm" />
                                                    <span className="text-[9px] font-bold text-zinc-400 uppercase mt-1">Toppbil</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="hidden md:flex flex-row items-center w-full min-w-0 relative">
                                            <div className="flex items-center gap-4 w-[30%] min-w-0 pr-4">
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0 border ${avatarTheme}`}>
                                                    {initials}
                                                </div>
                                                <span className="text-[14px] font-bold text-zinc-900 dark:text-white truncate group-hover:text-orange-500 transition-colors flex items-center gap-1.5">
                                                    <span className="truncate">{customer.name}</span> 
                                                    {customer.rank === 'Premium' && <SafeIcon name="crown" size={14} className="text-orange-500 mb-0.5 shrink-0" />}
                                                </span>
                                            </div>
                                            ...
                                            <div className="w-[20%] flex flex-col justify-center">
                                                <span className="text-[14px] font-light text-zinc-900 dark:text-white leading-none mb-1">
                                                    {customer.missionCount} <span className="text-[10px] font-bold text-zinc-400 uppercase">Besök</span>
                                                </span>
                                                <div className="flex items-center gap-1.5">
                                                    <SafeIcon name="truck" size={10} className="text-zinc-400" />
                                                    <span className="text-[10px] font-bold text-zinc-500 uppercase">{customer.vehicles.size} Fordon</span>
                                                </div>
                                            </div>

                                            <div className="w-[15%] text-right">
                                                <span className="text-[18px] font-light tracking-tighter text-zinc-900 dark:text-white">
                                                    {customer.totalSpent.toLocaleString()}
                                                </span>
                                                <span className="text-[10px] font-bold text-zinc-400 uppercase ml-1">kr</span>
                                            </div>

                                            <div className="flex items-center justify-end w-[20%] pr-4 relative">
                                                <div className="flex flex-col items-end mr-6 group-hover:opacity-0 group-hover:translate-x-4 transition-all duration-300">
                                                    <span className="text-[14px] font-light text-zinc-900 dark:text-white leading-none mb-1">
                                                        {daysSinceLast} <span className="text-[10px] font-bold text-zinc-400 uppercase">Dagar</span>
                                                    </span>
                                                    <div className="flex items-center gap-1.5">
                                                        <span className={`w-1.5 h-1.5 rounded-full ${isActive ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                                                        <span className="text-[9px] font-bold text-zinc-500 uppercase">{isActive ? 'Aktiv' : 'Inaktiv'}</span>
                                                    </div>
                                                </div>
                                                
                                                {/* FIX: En elegant gradient-fade bakgrund gör att Nytt Uppdrag-knappen ALDRIG krockar med omsättningen */}
                                                <div className="absolute right-0 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-end pr-4 pl-12 bg-gradient-to-l from-zinc-50 via-zinc-50 to-transparent dark:from-[#1f2940] dark:via-[#1f2940] w-64 h-full pointer-events-none group-hover:pointer-events-auto">
                                                    <div className="flex items-center gap-2">
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); setEditingJob(null); window.prefillName = customer.name; setView('NEW_JOB'); }}
                                                            className="bg-white dark:bg-[#1a2235] text-orange-500 hover:bg-orange-500 hover:text-white px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-widest border border-zinc-200 dark:border-white/10 hover:border-orange-500 transition-all shadow-sm"
                                                        >
                                                            + Nytt Uppdrag
                                                        </button>
                                                        <div className="w-10 h-10 rounded-full bg-zinc-100 dark:bg-[#1a2235] flex items-center justify-center group-hover:bg-orange-500 group-hover:text-white transition-colors border border-zinc-200 dark:border-white/10 hover:border-orange-500">
