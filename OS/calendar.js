...
window.CalendarView = ({ allJobs = [], setEditingJob, setView }) => {
    ...
    return (
        <div className="space-y-4 animate-in fade-in duration-500 pb-20 md:pb-0 bg-transparent transition-colors duration-300" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 pb-6 border-b border-zinc-200 dark:border-[#1a2235] gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                    <div className="flex bg-white dark:bg-[#121826] p-1 border border-zinc-200 dark:border-[#1a2235] rounded-[4px] shadow-sm dark:shadow-none">
                        {['WEEK', 'MONTH'].map(m => (
                            <button key={m} onClick={() => setViewMode(m)} className={`px-4 py-2 text-[10px] font-black uppercase tracking-wider rounded-[2px] transition-all flex-1 sm:flex-none ${viewMode === m ? 'bg-black dark:bg-[#252f48] text-white' : 'text-zinc-500 hover:text-black dark:hover:text-white'}`}>
                                {m === 'WEEK' ? 'Vecka' : 'Månad'}
                            </button>
                        ))}
                    </div>
                    <div className="flex gap-2">
                        <button onClick={() => changeDate(viewMode === 'WEEK' ? -7 : -30)} className="p-2.5 bg-white dark:bg-[#121826] text-zinc-900 dark:text-white border border-zinc-200 dark:border-[#1a2235] rounded-[4px] hover:border-orange-500 dark:hover:border-orange-500 transition-all shadow-sm dark:shadow-none flex-1 sm:flex-none flex justify-center">
            ...
            {/* GRID */}
            <div className="bg-zinc-50 dark:bg-[#0a0f18] border border-zinc-200 dark:border-[#1a2235] shadow-sm dark:shadow-none overflow-hidden rounded-[4px]">
                <div className="grid grid-cols-7 divide-x divide-zinc-200 dark:divide-[#1a2235]">
                    {calendarDays.map(({ date, isCurrentMonth }, i) => {
                        ...
                        return (
                            <div key={i} onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDrop(e, dStr)} className={`min-w-0 flex flex-col transition-all border-b border-zinc-200 dark:border-[#1a2235] ${viewMode === 'WEEK' ? 'min-h-[500px] md:min-h-[650px]' : 'min-h-[100px] md:min-h-[150px]'} ${!isCurrentMonth ? 'bg-zinc-50/30 dark:bg-[#0a0f18]/50' : 'dark:bg-[#0a0f18]'} ${today ? 'bg-white dark:bg-[#121826]' : ''}`}>
                                <div className={`p-2 md:p-4 border-b dark:border-[#1a2235] transition-colors ${today ? 'bg-zinc-950 dark:bg-[#1a2235]' : 'bg-zinc-100/50 dark:bg-[#121826]'}`}>
                                ...
                                <div className="p-1 md:p-3 space-y-1.5 md:space-y-3 flex-1 bg-white/50 dark:bg-transparent relative cursor-crosshair" onClick={() => setView('NEW_JOB', { job: { datum: `${dStr}T08:00`, status: 'BOKAD' } })}>
                                    {jobs.map(job => (
                                        <div key={job.id} draggable="true" onDragStart={(e) => onDragStart(e, job)} onDragEnd={(e) => e.currentTarget.style.opacity = '1'} onClick={(e) => { e.stopPropagation(); setView('NEW_JOB', { job: job }); }} className="group relative bg-white dark:bg-[#1a2235] border border-zinc-200 dark:border-[#2a3441] p-1.5 md:p-4 hover:border-zinc-900 dark:hover:border-zinc-500 transition-all cursor-grab active:cursor-grabbing shadow-sm dark:shadow-none hover:shadow-xl overflow-hidden rounded-sm">
                                            ...
                                            <div className="flex flex-col gap-0.5">
                                                ...
                                                <div className="flex flex-col md:flex-row md:items-center md:justify-between border-t border-zinc-50 dark:border-[#2a3441] pt-1 mt-1 gap-1">
                                                    ...
                                                    <div className="bg-zinc-900 dark:bg-[#0a0f18] text-white dark:text-zinc-300 px-1 py-0.5 font-black text-[6px] md:text-[9px] tracking-tighter self-start">{job.regnr}</div>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
