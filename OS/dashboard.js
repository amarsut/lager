...
// Unikt namn för att undvika SyntaxError: Identifier has already been declared
...
window.DashboardView = React.memo(({ 
    ...
    // Filter-komponent som används på både mobil och desktop
    const FilterButtons = () => (
        ['ALLA', 'BOKAD', 'OFFERERAD', 'EJ BOKAD', 'KLAR', 'FAKTURERAS'].map(s => (
            ...
            >
                ...
                <span className={`
                    ...
                    ${activeFilter === s ? 'bg-black/20' : 'bg-zinc-800 text-zinc-600'}
                `}>
                    {statusCounts[s] || 0}
                </span>
            </button>
    ...
    return (
        <div className="space-y-4 pb-20 lg:pb-0 animate-in fade-in duration-500">
            ...
            {/* MOBIL FILTRERING - Horisontell scrollbar som inte påverkar bredden på korten */}
            {!searchOpen && (
                <div className="lg:hidden px-4 overflow-x-auto no-scrollbar animate-in slide-in-from-top-2">
                    <div className="flex bg-zinc-950 p-1 border border-zinc-800 rounded-sm gap-1 w-max">
                        <FilterButtons />
                    </div>
                </div>
            )}
