...
// ============================================================================
const CarDetailsModal = ({ car, initialIndex = 0, onClose, setFinanceCar, calculateMonthlyCost, cmsData }) => {
    ...
    // --- DYNAMISK SEO (UPPDATERAR FLIK, GOOGLE & DELNINGSLÄNKAR) ---
    useEffect(() => {
        if (!car) return;

        // Spara originaltiteln för att kunna återställa när modalen stängs
        const origTitle = document.title;
        const metaDescEl = document.querySelector('meta[name="description"]');
        const origDesc = metaDescEl ? metaDescEl.content : '';

        // Hjälpfunktion för att skapa eller uppdatera meta-taggar (Open Graph för Facebook/SMS)
        ...
        // 1. Ändra flikens namn (Syns på Google och i webbläsaren)
        document.title = `${car.brand} ${car.model} | BMG Motorgrupp`;

        // 2. Ändra Google-beskrivningen
        const shortDesc = `Köp ${car.brand} ${car.model} hos oss. ${car.mil}, ${car.gear}. Pris: ${car.price}. Trygg affär hos BMG Motorgrupp i Eslöv.`;
        setMeta('name', 'description', shortDesc);

        // 3. Open Graph (Det som skapar "bild-rutan" när länken delas på Facebook/iMessage)
        setMeta('property', 'og:title', `${car.brand} ${car.model} - ${car.price}`);
        setMeta('property', 'og:description', `${car.mil} | ${car.gear} | ${car.fuel}`);
        setMeta('property', 'og:image', car.images[0]);
        setMeta('property', 'og:url', window.location.href);

        // Återställ till standardsidan när modalen stängs
        return () => {
            document.title = origTitle;
            if (metaDescEl) metaDescEl.content = origDesc;
        };
    }, [car]);

    // --- NY BILD-LOGIK SOM LÖSER CACHE-BUGGEN ---
    useEffect(() => { 
        setImgLoaded(false); 
    ...
    const handleInterestSubmit = async (e) => {
        ...
        try {
            ...
            if (cmsData?.sendLeadEmails !== false) {
                ...
                if (!mailRes.ok) throw new Error("Mejl misslyckades");
            }
            ...
            const firestoreBody = {
                fields: {
                    ...
                    subject: { stringValue: formData.get("subject") || "Intresseanmälan" },
                    status: { stringValue: "new" },
                    createdAt: { timestampValue: new Date().toISOString() }
                }
            };

            const dbRes = await fetch("https://firestore.googleapis.com/v1/projects/bmg-motorgrupp/databases/(default)/documents/leads", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(firestoreBody)
            });

            if (!dbRes.ok) throw new Error("Databas misslyckades");

            // 3. Visa framgångsmeddelande i modalen
            setFormSent(true);
                ...
                <div className="w-full lg:w-[55%] h-[40vh] lg:h-full bg-black relative flex flex-col shrink-0">
                    ...
                    <div className="flex-1 relative flex items-center justify-center group overflow-hidden">
                        ...
                        <img 
                            ...
                            key={currentImg} // <--- Viktigt att key är kvar!
                        />
...
const CarCard = ({ car, handleShare, setFinanceCar, calculateMonthlyCost, onSelectCar, cmsData }) => {
    const [currentImg, setCurrentImg] = useState(0);
    const [imgLoaded, setImgLoaded] = useState(false);
    const imgRef = useRef(null); // <-- NY REF

    // --- NY BILD-LOGIK HÄR OCKSÅ ---
    useEffect(() => { 
        setImgLoaded(false); 
        if (imgRef.current && imgRef.current.complete) {
            setImgLoaded(true);
        ...
        >
            <div className="h-64 overflow-hidden relative bg-black flex items-center justify-center shrink-0 group/slider">
                ...
                <img
                    ...
                    key={currentImg} // <-- LÄGG TILL KEY HÄR OCKSÅ!
                />
...
const LagerSection = ({ setFinanceCar, cmsData }) => {
    ...
    // --- HÄMTA BILAR (REST API VERSION - KRASCHSÄKER) ---
    useEffect(() => {
        const fetchCars = async () => {
            setLoadingCars(true);
            setApiError(false);
            try {
                // 1. Hämta lagret via standard-fetch (samma metod som i app.js)
                const response = await fetch("https://firestore.googleapis.com/v1/projects/bmg-motorgrupp/databases/(default)/documents/system/inventory");
                
                if (!response.ok) {
                    throw new Error("Kunde inte hämta lagret från databasen.");
                }
                
                const json = await response.json();

                // 2. Hjälpfunktion för att packa upp Firebase REST-formatet till vanliga JS-objekt
                const parseFirestore = (val) => {
                    if (!val) return null;
                    ...
                    if (val.arrayValue !== undefined) return (val.arrayValue.values || []).map(parseFirestore);
                    if (val.mapValue !== undefined) {
                        const res = {};
                        for (const k in val.mapValue.fields) res[k] = parseFirestore(val.mapValue.fields[k]);
                        return res;
                    }
                    return val;
                };

                // 3. Packa upp bilarna (Om .cars finns, annars en tom array)
                const data = json.fields?.cars ? parseFirestore(json.fields.cars) : [];

                // 4. FORMATERA BILARNA FÖR HEMSIDAN (Samma som innan)
    ...
    const calculateMonthlyCost = (priceStr) => {
        ...
        // Använd datan från CMS eller defaultvärden
        const interest = (cmsData?.financeInterest || 7.95) / 100;
        const downPaymentPercent = (cmsData?.financeDownpay || 20) / 100;
        const years = 6;
    ...
    const openCarModal = async (carData, index) => {
        setSelectedCarData({ car: carData, index });
        window.history.pushState({}, '', `?bil=${carData.id}`);

        try {
            // Den exakta sökvägen till dokumentet i Firebase
            const docName = `projects/bmg-motorgrupp/databases/(default)/documents/car_stats/${carData.id}`;
            
            // 1. Fråga databasen: Vad är siffran just nu?
            const getRes = await fetch(`https://firestore.googleapis.com/v1/${docName}`);
            let currentViews = 0;
            
            if (getRes.ok) {
                const data = await getRes.json();
                currentViews = parseInt(data.fields?.views?.integerValue || 0);
            }

            // 2. Skicka den nya siffran med POST (Detta undviker CORS-kraschen!)
            const newViews = currentViews + 1;
            
            await fetch("https://firestore.googleapis.com/v1/projects/bmg-motorgrupp/databases/(default)/documents:commit", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    writes: [{
                        update: {
                            name: docName,
                            fields: {
                                views: { integerValue: newViews.toString() }
                            }
                        }
                    }]
                })
            ...
            console.error("Kunde inte spara klickstatistik:", e);
        }
    };
        ...
        <section id="lager" className="py-24 bg-brand-950 relative z-10">
            ...
            <div className="max-w-[1400px] mx-auto px-6 relative z-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
                    <div>
                        <h2 className="text-4xl md:text-6xl font-black text-white tracking-tight mb-4">Vårt urval.</h2>
                        <p className="text-slate-400 text-lg md:text-xl max-w-2xl">Utforska vårt noggrant testade utbud av fordon, redo för omedelbar leverans.</p>
                    </div>
