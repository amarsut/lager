...
const App = () => {
            ...
            try {
                ...
                if (json.fields) {
                    const data = {
                        ...
                        bannerText: json.fields.bannerText?.stringValue || "",
                        sendLeadEmails: json.fields.sendLeadEmails?.booleanValue ?? true,
                        showFinance: json.fields.showFinance?.booleanValue ?? true,
                        showBanner: json.fields.showBanner?.booleanValue ?? false,
                        financeInterest: json.fields.financeInterest?.doubleValue ?? (json.fields.financeInterest?.integerValue ?? 7.95),
                        financeDownpay: json.fields.financeDownpay?.integerValue ?? (json.fields.financeDownpay?.doubleValue ?? 20),
                        gaId: json.fields.gaId?.stringValue || "",
                        pixelId: json.fields.pixelId?.stringValue || "",
                        reviewEmbedCode: json.fields.reviewEmbedCode?.stringValue || "",
                        useGoogleEmbed: json.fields.useGoogleEmbed?.booleanValue ?? false,
                        specialHours: json.fields.specialHours?.stringValue || "", // Avvikande öppettider
                        reviews: json.fields.reviews?.arrayValue?.values?.map(v => ({
    ...
    useEffect(() => {
        ...
        if (cmsData.gaId) {
            ...
            script1.async = true;
            script1.src = `https://www.googletagmanager.com/gtag/js?id=${cmsData.gaId}`;
        ...
        if (cmsData.pixelId) {
            ...
            pixelScript.innerHTML = `
                ...
                fbq('init', '${cmsData.pixelId}');
                fbq('track', 'PageView');
            `;
            document.head.appendChild(pixelScript);
        }
    }, [cmsData?.gaId, cmsData?.pixelId]);
    ...
    return (
        <div className="min-h-screen flex flex-col font-sans relative overflow-x-hidden">
            ...
            {/* --- NAVBAR --- */}
            <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled || mobileMenuOpen ? 'bg-brand-950/95 backdrop-blur-md shadow-lg border-b border-white/5' : 'bg-transparent'}`}>
                {/* INFO BANNER */}
                {cmsData?.showBanner && cmsData?.bannerText && (
                    <div className="bg-brand-500 text-white text-center py-2 px-4 text-sm font-bold tracking-wide relative z-50 shadow-md flex items-center justify-center gap-2">
                        <window.Icon name="info" size={16} /> {cmsData.bannerText}
                    </div>
                )}
                <div className={`max-w-7xl mx-auto px-6 flex justify-between items-center relative z-20 ${cmsData?.showBanner ? 'py-3' : (scrolled || mobileMenuOpen ? 'py-4' : 'py-6')}`}>
                    ...
                    <div className="hidden md:flex gap-8 text-sm font-semibold tracking-wide items-center">
                        {/* Ny, stilren Bilar i lager-knapp */}
                        <a href="#lager" className="bg-brand-500 text-white px-5 py-2.5 rounded-lg hover:bg-brand-600 transition-colors shadow-md shadow-brand-500/20 focus-visible:ring-2 focus-visible:ring-white outline-none flex items-center gap-2 font-bold">
                            <window.Icon name="car" size={18} /> Bilar i lager
                        </a>
            ...
            <main>
                ...
                {typeof window.LagerSection !== 'undefined' ? <window.LagerSection setFinanceCar={setFinanceCar} cmsData={cmsData} /> : <LagerSection setFinanceCar={setFinanceCar} cmsData={cmsData} />}
                ...
                <section id="recensioner" className="py-24 bg-brand-900 border-t border-white/5">
                    <div className="max-w-7xl mx-auto px-6">
                        ...
                        {cmsData?.useGoogleEmbed && cmsData?.reviewEmbedCode ? (
                            <div 
                                className="w-full flex justify-center" 
                                dangerouslySetInnerHTML={{ __html: cmsData.reviewEmbedCode }} 
                            />
                        ) : (
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                {(cmsData?.reviews?.length > 0 ? cmsData.reviews : (typeof reviews !== 'undefined' ? reviews : [])).map((review, idx) => (
                    ...
                    <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16">
                        ...
                        <div className="space-y-8">
                            <div className="bg-brand-900 p-8 rounded-2xl border border-white/5 shadow-xl">
                                ...
                                <form className="space-y-4" onSubmit={async (e) => { 
                                    ...
                                    try {
                                        ...
                                        if (cmsData?.sendLeadEmails !== false) {
                                            ...
                                            if (!mailResponse.ok) throw new Error("Kunde inte skicka mejl via Web3Forms");
                                        }
                            ...
                            <div className="bg-brand-900 p-8 rounded-2xl border border-white/5 shadow-xl">
                                ...
                                {cmsData?.specialHours && (
                                    <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl mb-6 flex items-start gap-3">
                                        <window.Icon name="alert-triangle" className="shrink-0 mt-0.5" size={20} />
                                        <div>
                                            <strong className="block text-xs uppercase tracking-widest mb-1 font-black">Avvikande Tider</strong>
                                            <p className="text-sm font-semibold">{cmsData.specialHours}</p>
                                        </div>
                                    </div>
                                )}
                                {/* AVVIKANDE ÖPPETTIDER SLUT */}
                            ...
                            <div className="p-6 sm:p-8 bg-brand-950 border-t border-white/5">
                                ...
                                <form 
                                    ...
                                    onSubmit={async (e) => {
                                        ...
                                        try {
                                            ...
                                            // Skicka mejl BARA om inställningen tillåter det
                                            if (cmsData?.sendLeadEmails !== false) {
                                ...
                                >
                                    <input 
                                        ...
                                        className="w-full bg-brand-900 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm uppercase placeholder:normal-case font-bold" 
                                    />
                                    <input 
                                        type="email" 
                                        name="email"
                                        ...
                                        className="w-full bg-brand-900 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm" 
                                    />
                                    <input 
                                        type="tel" 
                                        name="phone"
                                        placeholder="Telefonnummer (Frivilligt)" 
                                        className="w-full bg-brand-900 border border-white/10 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all text-sm placeholder:text-slate-500" 
                                    />
                                    <button type="submit" className="w-full bg-brand-500 hover:bg-brand-600 text-white px-6 py-3 rounded-lg font-bold transition-all text-sm flex items-center justify-center gap-2 whitespace-nowrap outline-none focus-visible:ring-2 focus-visible:ring-white">
